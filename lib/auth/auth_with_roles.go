...
// GetConnectionDiagnostic returns all connection diagnostics that match the filter.
func (a *ServerWithRoles) GetConnectionDiagnostic(ctx context.Context, name string) (types.ConnectionDiagnostic, error) {
	if err := a.action(apidefaults.Namespace, types.KindConnectionDiagnostic, types.VerbList, types.VerbRead); err != nil {
		return nil, trace.Wrap(err)
	}
