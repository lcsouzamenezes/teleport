...
test-go: $(VERSRC) $(TEST_LOG_DIR)
	$(CGOFLAG_TSH) go test -cover -json -tags "$(PAM_TAG) $(FIPS_TAG) $(LIBFIDO2_TEST_TAG) $(TOUCHID_TAG)" -run ^TestTSHProxyTemplate$$ -v github.com/gravitational/teleport/tool/tsh $(FLAGS) $(ADDFLAGS) \
